...
public class MyHorizontalScrollViewBelow extends HorizontalScrollView implements
        OnClickListener {
    //敏感度，自动调整位置的幅度
    private static int SENSITIVENESS = 200;
    private boolean MotionLock = true;
    ...
    private static final String TAG = "MyHorizontalScrollViewBelow";
     ...
     * 数据适配器
     */
    private HorizontalScrollViewAdapterBelow mAdapter;
    /**
     * 每屏幕最多显示的个数
     */
    ...
    public MyHorizontalScrollViewBelow(Context context, AttributeSet attrs) {
        super(context, attrs);
        // 获得屏幕宽度
    ...
    public void notifyCurrentImgChanged() {
        ...
        for (int i = 0; i < mContainer.getChildCount(); i++) {
            ...
            TextView vii = (TextView) mContainer.getChildAt(i).findViewById(R.id.gallery_item_below);
            vii.setTextColor(Color.GRAY);
        }
     ...
     * @param mAdapter
     */
    public void initDatas(HorizontalScrollViewAdapterBelow mAdapter) {
    ...
    public void initFirstScreenChildren(int mCountOneScreen) {
        ...
        for (int i = 0; i < mContainer.getChildCount(); i++) {
            mContainer.getChildAt(i).setBackgroundColor(Color.BLACK);
            TextView v = (TextView) mContainer.getChildAt(i).findViewById(R.id.gallery_item_below);
    ...
    public boolean onTouchEvent(MotionEvent ev) {switch (ev.getAction()) {
        case MotionEvent.ACTION_MOVE:
            if(scroll_flag){  Log.i("scroll_flag", "我应该滚到到mChildWidth");
                FrameLayout.LayoutParams lp = new FrameLayout.LayoutParams(LayoutParams.WRAP_CONTENT,DensityUtil.dip2px(this.getContext(),80));
    ...
    public void onClick(View v) {
        if (mOnClickListener != null) {
            for (int i = 0; i < mContainer.getChildCount(); i++) {
                ...
                TextView vi = (TextView) mContainer.getChildAt(i).findViewById(R.id.gallery_item_below);
                vi.setTextColor(Color.parseColor("#a9b7b7"));
            }
            mOnClickListener.onClick(v, mViewPos.get(v));
            TextView view = (TextView) v.findViewById(R.id.gallery_item_below);
